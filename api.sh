...
then
   ...
   anypoint-cli --username="$anypoint_usr" --password="$anypoint_pws" --organization="$Business_Group" --environment="$Target_Environment" api-mgr api change-specification -o json $API_InstanceId $API_Asset_Version
   
elif [ $Actions == "Promote" ]
then
   anypoint-cli --username="$anypoint_usr" --password="$anypoint_pws" --organization="$Business_Group" --environment="$Target_Environment" api-mgr api promote -o json --copyPolicies true $API_InstanceId "da669ba5-e2b1-41ff-9bff-f1350361776d"
else
